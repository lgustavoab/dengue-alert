"""Treina e avalia o Modelo A epidemiológico do Dengue Alert."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=("Treina e avalia o Modelo A epidemiológico.")
    )
...
def output_paths(
    ...
    return (
        output_dir / f"avaliacao_modelo_a{suffix}.json",
        output_dir / f"avaliacao_modelo_a{suffix}.csv",
    )


def required_columns() -> list[str]:
    """Retorna somente as colunas necessárias ao Modelo A."""
    columns = [
        MUNICIPALITY_COLUMN,
        YEAR_COLUMN,
        "data_inicio_semana",
...
def write_outputs(
    *,
    ...
    report = {
        "status": status,
        "modo": mode,
        "modelo_conceitual": "Modelo A — epidemiológico",
        "features": {
...
def main() -> None:
    """Executa o treinamento temporal do Modelo A."""
    args = parse_args()

    mode = args.mode

    print("=" * 88)
    print("MODELO A — FEATURES EPIDEMIOLÓGICAS")
    print("=" * 88)
